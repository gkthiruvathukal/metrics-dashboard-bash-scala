...
object KlocSysScala extends IO with GitBashExec{
  ...
  def main(args:Array[String]): Unit ={
    ...
    val result = gitLogSpark.foreach(x =>{
      ...
      Seq("/bin/sh", "-c", "cd literalinclude-scala && git checkout "+sha+ " && cloc --by-file --report_file=res2.txt ../literalinclude-scala/ && grep \"../\" res2.txt >> ~/log100.txt && echo "+date+" >> ~/log100.txt" ).! //&& " +
